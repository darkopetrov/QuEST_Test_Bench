...
comp_QuEST = "/home/devel/QuEST-gpu/build/grover"

OUTPUT_DIR = "output_GPU_exec_time"
...
def run_mem_check(cmd, stdout_file):
    ...
    out = subprocess.check_output(["heaptrack_print", "-p", "-f", heaptrack_file, "|", "grep", "peak heap memory consumption:"])
        ...
        for block_size in test_cases[qubits]["b"]:
            for param in test_cases[qubits]["params"]:
                ...
                with open(os.path.join(output_path, f"test_case_file_{test_case_file}"), "w") as f:
                    ...
                    f.write(f"peak mem usage: {peak_mem} ns\n")
...
if __name__ == "__main__":
    mem_test = False
    run_zfp_test_cases(mem_test)
    run_fpzip_test_cases(mem_test)
    #run_original_test_cases(mem_test)